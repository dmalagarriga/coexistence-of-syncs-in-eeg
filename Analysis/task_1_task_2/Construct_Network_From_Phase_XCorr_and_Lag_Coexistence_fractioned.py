...
for k in range(len(Annotations)):
	...
	if(plot_matrices==1):
		...
		plt.savefig(pjoin(path_network,'Complete_Sync_Matrix_Average_Coexistence_%s.pdf' %state))
		plt.close()
		...
		#plt.cbar=plt.colorbar(ticks=[0.0, 0.5, 1.0])
		plt.cbar.ax.set_ylabel('Frequency')
		plt.savefig(pjoin(path_network,'Lag_Sync_Matrix_Average_Coexistence_%s.pdf' %state ))
		plt.close()
		...
		plt.cbar.ax.set_ylabel('Frequency')
		plt.clim=([0.0,1.0])
		plt.savefig(pjoin(path_network,'Phase_Sync_Matrix_Average_Coexistence_%s.pdf' %state ))
		plt.close()
	...
	if (plot_network==1):
		...
		for i in range(len(nx.strongly_connected_components(G))):
			if len(nx.connected_component_subgraphs(G)[i])>=1:
		...
		pylab.savefig('%s/Network_Reconstructed_XCORR_PHASE_LAG_Coexistence_Thresh_%s_%s.pdf' %(path_network,thresh,state),bbox_inches='tight')
		pylab.close()
